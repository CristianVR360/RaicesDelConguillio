  ...
  try {
    ...
    if (panorama) {
      if (panorama.hotspots && panorama.hotspots.length > 0) {
        ...
        console.warn('Panorama with id "node2" has no hotspots');
        throw new Error(`Panorama with id "node2" has no hotspots`);
      }
    } else {
      console.warn('Panorama with id "node2" not found');
      throw new Error(`Panorama with id "node2" not found`);
    }
  } catch (error) {
    console.error(error);
    return error.message;
